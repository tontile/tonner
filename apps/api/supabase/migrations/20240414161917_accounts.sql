...
-------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_account_registry_state()
    RETURNS json
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    RETURN json_build_object('total_count', total_count, 'organization_count', org_count, 'user_count', user_count);
END;
$$;
