...
DO $$
BEGIN
    -- check it invitation_type already exists on app schema
    IF NOT EXISTS(
        SELECT
            1
        FROM
            ...
            JOIN pg_namespace n ON n.oid = t.typnamespace
        WHERE
            t.typname = 'invitation_type'
            AND n.nspname = 'app') THEN
    CREATE TYPE app.invitation_type AS ENUM(
        'one_time',
        '24_hour'
);
END IF;
...
-------------------------------------------------------
CREATE OR REPLACE FUNCTION app.generate_name(account_name text, partial_name text DEFAULT NULL)
...
GRANT EXECUTE ON FUNCTION app.generate_name(text, text) TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION app.generate_hmac(secret_key text, message text)
    RETURNS text
    LANGUAGE plpgsql
    AS $$
DECLARE
...
GRANT EXECUTE ON FUNCTION app.generate_hmac(text, text) TO authenticated, service_role;
